...
> >
> > ![1537711706217](assets/1537711706217.png)
...
考虑当$x=-(1e-6)$的时, 对ReLU函数进行梯度检查. 因为![x<0](https://www.zhihu.com/equation?tex=x%3C0), 所以解析梯度在该点的梯度为0. 然而, 在这里数值梯度会突然计算出一个非零的梯度值, 因为![f(x+h)](https://www.zhihu.com/equation?tex=f%28x%2Bh%29)可能越过了不可导点(例如：如果![h>1e-6](https://www.zhihu.com/equation?tex=h%3E1e-6)), 导致了一个非零的结果.
...
第一层数据可视化,可以看出来第一层在寻找些什么.

这里灵感来源于模板匹配和内积:可以想象,有一些模板向量,然后通过模板向量和一些任意数据之间的点积,得到标量输出,然后当这两个向量相互匹配时,输入将在范数约束的条件下,得到**最大输出**.
...
> ~~因为对于海森矩阵的运算这里多了一项倒数~~这里是矩阵求逆, 那该如何理解？
>
> 应该是类似的理解方式, 矩阵求逆和倒数有一定的相似性.
...
例如：当学习率是0.001的时候, 如果对其固定地增加0.01, 那么对于学习进程会有很大影响. 然而当学习率是10的时候, 影响就微乎其微了.

这就是因为学习率乘以了计算出的梯度.

> 这里怎么理解？
>
> 可以这样理解：因为加减特定的值, 由于学习率本身的尺度大小, 会使得加减所产生的影响效果不同, 而乘法除法对于所产生的影响也是成比例变化.
...
![1538105042278](assets/1538105042278.png)

> https://wenku.baidu.com/view/3736f20d24c52cc58bd63186bceb19e8b8f6ece1.html
