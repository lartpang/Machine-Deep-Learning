...
这些工作仍采用与DCNN分类器结果分离的分割算法, 因此有可能承担过早的决策(thus risking  commitment to premature decisions).
...
* 最初由[The discrete wavelet transform: wedding the a trous and mallat algorithms]提出并在[OverFeat]中使用的第二种方法, 是通过等价于带洞卷积率r的因子对输入特征图进行下采样. 将其隔行扫描产生大小为$r^2$的降分辨率的图, 每个图有一个rxr可能的转变. 然后将标准卷积应用于这些中间特征图并将它们重新交换为原始图像分辨率. 通过将带洞卷积减少为常规卷积, 它允许用现成的(off-the-shelf)高度优化的卷积方式.
