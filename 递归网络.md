...
>
>    则对$\frac{\part}{\part c}$以及$\frac{\part}{\part V}$而言，就是如同logistic函数的损失函数对于$\theta_0\&\theta_j$的求导。结果就是预测值与真实值的差值，以及多出来的一项$h^{(t)}$。
>
> 2. 最后的那个$T$是怎么来的？是矩阵转置么？
>
...
> 这里要注意第二个因式。
>
> $h^{(t)}=σ(z^{(t)})=σ(Ux^{(t)}+Wh^{(t−1)}+b)$——在此基础上考虑，再结合$tanh$的倒数，大体不用推到也可理解。
...
输入新的,会忘记一些旧的,忘记一些旧的,会输入一些新的.(这里描述输入/忘记的量的衡量是给予系数$f$而言的,应该是这样.)
